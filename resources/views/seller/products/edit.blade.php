<x-seller-layout>
    <div>
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-gray-900 font-serif">Edit Produk</h1>
            <p class="text-purple-600">Perbarui informasi produk Anda</p>
        </div>
        ...
        <div class="bg-white rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-100 max-w-3xl">
            <form action="{{ route('seller.products.update', $product) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    <h2 class="text-lg font-medium text-gray-900 mb-4">Foto Produk</h2>
                    
                    @if($product->images->count() > 0)
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
                            @foreach($product->images as $image)
                                <div class="relative group overflow-hidden rounded-xl border border-gray-200 aspect-w-1 aspect-h-1">
                                    <img src="{{ asset('storage/' . $image->image_path) }}" alt="Product Image" class="w-full h-32 object-cover transition-transform duration-300 group-hover:scale-110">
                                    
                                    <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                                        <button type="button" onclick="confirmDeleteImage('delete-image-{{ $image->id }}')" class="transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300 bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg shadow-lg flex items-center gap-2 font-medium text-sm">
                    ...
                    <div x-data="imagePreview()">
                        <x-input-label for="images" :value="__('Tambah Foto (Max 5 total)')" class="text-gray-700 font-medium" />
                    ...
                    <script>
                        // Function for deleting existing images (from DB)
                ...
                <div>
                    ...
                    <x-text-input id="name" name="name" type="text" class="mt-1 block w-full rounded-lg border-gray-300 focus:border-purple-500 focus:ring-purple-500" :value="old('name', $product->name)" required autofocus />
                ...
                <div>
                    ...
                    <div class="relative mt-1 rounded-md shadow-sm">
                        ...
                        <x-text-input id="price" name="price" type="number" class="block w-full rounded-lg border-gray-300 pl-10 focus:border-purple-500 focus:ring-purple-500" :value="old('price', $product->price)" required />
                    </div>
                ...
                <div>
                    ...
                    <x-text-input id="category" name="category" type="text" class="mt-1 block w-full rounded-lg border-gray-300 focus:border-purple-500 focus:ring-purple-500" :value="old('category', $product->category)" required placeholder="Contoh: Makanan Ringan" />
                ...
                <div>
                    ...
                    <textarea id="description" name="description" class="mt-1 block w-full border-gray-300 focus:border-purple-500 focus:ring-purple-500 rounded-lg shadow-sm" rows="4" required>{{ old('description', $product->description) }}</textarea>
                ...
                <div>
                    ...
                    <x-text-input id="whatsapp_number" name="whatsapp_number" type="text" class="mt-1 block w-full rounded-lg border-gray-300 focus:border-purple-500 focus:ring-purple-500" :value="old('whatsapp_number', $product->whatsapp_number ?? auth()->user()->shop_whatsapp)" placeholder="628xxxxxxxxxx" />
        ...
        <!-- Hidden Delete Forms -->
        @foreach($product->images as $image)
            <form id="delete-image-{{ $image->id }}" action="{{ route('seller.products.image.destroy', $image->id) }}" method="POST" style="display: none;">
                @csrf
                @method('DELETE')
            </form>
        @endforeach
